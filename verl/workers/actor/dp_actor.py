...
class DataParallelPPOActor(BasePPOActor):
    ...
    def compute_gradients(self, data: DataProto):
        """
        对当前 batch 做 forward+backward，不更新参数，只返回梯度向量。
        用于 Lu et al. gradient-based 动态权重计算。
        """
        ...
        # 和 update_policy 一样拆 micro_batch
        ...
        for mb in micro_batches:
            ...
            entropy, log_prob = self._forward_micro_batch(mb, temperature)
        ...
        full_grad = torch.cat(flat_grads)

        # 清零，不做 optimizer.step()
        self.actor_optimizer.zero_grad()
