...
class ActorRolloutRefWorker(Worker):
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        return output
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_gradients(self, data: DataProto):
        """在 FSDPWorker 层级转发梯度计算请求"""
        assert self._is_actor, "Gradient computation requires actor role"
        
        # 1. 将模型和优化器搬到 GPU (如果开启了 offload)
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        if self._is_offload_optimizer:
            load_fsdp_optimizer(self.actor_optimizer, device_id=get_torch_device().current_device())

        # 2. 处理并行分片并调用底层的 DPActor
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
            
            # 注意：确保这里的调用和你在 dp_actor.py 定义的签名一致
            # 如果你的 dp_actor 不支持 target_layer_ids，请删掉它
            flat_grads = self.actor.compute_gradients(data=data)

            # 3. 将结果包装回 DataProto 以便在分布式节点间传输
            output = DataProto.from_dict(tensors={"flat_grad": flat_grads})
            output = self.ulysses_sharding_manager.postprocess_data(output)
