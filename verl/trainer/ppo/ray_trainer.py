...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1):
    ...
    if adv_estimator == 'gae':
    ...
    elif adv_estimator == 'grpo-h':
        token_level_scores_correctness = data.batch['token_level_scores_correctness']
        token_level_scores_format = data.batch['token_level_scores_format']
        index = data.non_tensor_batch['uid']
        response_mask = data.batch['attention_mask'][:, -data.batch['responses'].size(-1):]
        with torch.no_grad():
            seq_scores_corr = (token_level_scores_correctness * response_mask).sum(dim=-1)
            seq_scores_fmt = (token_level_scores_format * response_mask).sum(dim=-1)
            offset_corr = seq_scores_corr + 3.0
            offset_fmt = seq_scores_fmt 
            mu_c_off, std_c = offset_corr.mean(), offset_corr.std()
            mu_f_off, std_f = offset_fmt.mean(), offset_fmt.std()
            eps = 1e-8
        ...
        combined_rewards = (w_corr * token_level_scores_correctness) + \
                           (w_fmt * token_level_scores_format)
        advantages, returns = core_algos.compute_grpo_outcome_advantage(
            token_level_rewards=combined_rewards,
            eos_mask=response_mask,
            index=index
        )
        ...
        with torch.no_grad():
            seq_scores_corr = (token_level_scores_correctness * response_mask).sum(dim=-1)
            seq_scores_fmt = (token_level_scores_format * response_mask).sum(dim=-1)
            offset_corr_scores = seq_scores_corr + 3.0
            mu_c, std_c = offset_corr_scores.mean(), offset_corr_scores.std()
            mu_f, std_f = seq_scores_fmt.mean(), seq_scores_fmt.std()
            epsilon = 1e-8
        ...
        fmt_adv, _ = core_algos.compute_grpo_outcome_advantage(
            token_level_rewards=token_level_scores_format,
            eos_mask=response_mask, index=index
        )
        combined_adv = (w_corr * corr_adv) + (w_fmt * fmt_adv) 
        advantages = masked_whiten(combined_adv, response_mask) * response_mask
        data.batch['advantages'] = advantages
        data.batch['returns'] = advantages
    else:
...
class RayPPOTrainer(object):
    ...
    def init_workers(self):
        ...
        if self.config.algorithm.adv_estimator == 'gae':
            ...
            self.use_critic = False
        elif self.config.algorithm.adv_estimator == 'grpo-h':
            self.use_critic = False
        elif self.config.algorithm.adv_estimator == 'gdpo-h':
            self.use_critic = False
        else:
            raise NotImplementedError
